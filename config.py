...
<b>Список команд:</b>\n
► /start или /cancel 
<i>Экстренный возврат в главное меню.\nИспользуйте в любой непонятной ситуации.</i>\n
► /help 
<i>Просмотр справочных сообщений.</i>\n
...
Листайте вправо, чтобы ознакомиться с доступными командами.
"""
    HELP_MEDIA = """
<b>❕ Как использовать бота ❕</b>\n
1. Вернитесь в главное меню бота ("Закрыть" или /start).
2. Создайте пару новых категорий (Настройки -> Добавить категорию).
3. Отправьте любое сообщение, которое хотите сохранить.
...
4. Нажмите кнопку <b>Сохранить</b>.
5. Выберите категорию, в которую нужно сохранить запись, и (опционально) добавьте комментарий от себя.
...
class Replies:
    ...
    PART_RECEIVED_NO = 'Слишком много сообщений. Нажмите кнопку "Сохранить"'
    NO_TEMP = 'Чтобы сохранить что-то, нужно сначала это что-то отправить ☝️'
    CAPTION_TOO_LONG = '❌ Недопустимая длина подписи вложения.'
    COMMENT_RETRY = 'Не удалось обработать комментарий. Попробуйте еще ' \
                    '(или /start для отмены и возвращения в главное меню).'
    CHOOSE_CATEGORY = 'Выберите категорию из доступных:'
    HANDLE_CATEGORY_ERROR = '❌ Нужно нажать кнопку с категорией.'
    ...
    NEW_CATEGORY_NAME_ = 'Введите новое название для категории <code>{}</code>'
